...
TEST_F(NeblinaCoreFixture, addVectorF) {
    ...
    vector_t * r = m.bridges[idx].vector_new(n, T_FLOAT, 0, NULL );
    ...
    m.bridges[idx].vecreqdev(b);
    m.bridges[idx].vecreqdev(r);

    r->extra = m.bridges[idx].addVectorF_f((cl_mem)a->extra, (cl_mem)b->extra, b->len);
...
TEST_F(NeblinaCoreFixture, vec_add) {
    ...
    //This is called via neblina-core.so
...
TEST_F(NeblinaCoreFixture, addVectorC) {
    ...
    vector_t * r = m.bridges[idx].vector_new(n, T_COMPLEX, 0, NULL);
    ...
    r->extra = m.bridges[idx].addVectorC_f((cl_mem)a->extra, (cl_mem)b->extra, b->len);
...
TEST_F(NeblinaCoreFixture, subVector) {
    ...
    r->extra = m.bridges[idx].subVector_f((cl_mem)a->extra, (cl_mem)b->extra, b->len);
